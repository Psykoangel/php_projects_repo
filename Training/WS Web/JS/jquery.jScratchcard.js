			...
			return this.each(function(i) {
				...
				for (i=0;i<w;i+=o.stepx) {
					for (j=0;j<h;j+=o.stepy) {
						over += "<div id='e"+ (i/o.stepx) + "_"+ (j/o.stepy) +"' class='"+uuid+"' style='height:" + o.stepy + "px; width:" + o.stepx + "px; background-color:" + o.color + "; position: absolute;border: 0;overflow: hidden;top:" + (y + j) + "px; left:" + (x + i) + "px;' />";
					}
				}
				el.after(over);
				$('.'+uuid)
					.mouseover(function() {
						if (o.mousedown == false || el.data('okMouse')) {
							...
							if (op <= 0)
							{
								if(this.id == "e8_8" || this.id == "e8_7" || this.id == "e8_6" ||
								this.id == "e7_8" || this.id == "e7_7" || this.id == "e7_6") 
								{
									if(found == 5) {alert('Vous l\'avez trouvé !');}
									found++;
								}
								$(this).remove();
							}
